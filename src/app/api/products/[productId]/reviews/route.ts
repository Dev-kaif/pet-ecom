...
interface ProductReviewsRouteParams {
  params: { productId: string };
}

export async function GET(request: NextRequest, { params }: ProductReviewsRouteParams) {
  await dbConnect();
...
export async function POST(request: NextRequest, { params }: ProductReviewsRouteParams) {
  ...
  try {
    ...
    return NextResponse.json({ success: false, message: 'Failed to add review.', error: error.message }, { status: 500 });
  }
}