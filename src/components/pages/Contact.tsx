...
const ContactPage: React.FC = () => {
  ...
  const handleSubmit = (e: React.FormEvent) => {
    ...
    alert("Message sent successfully!"); // Simple alert for demonstration
    setFormData({ name: "", email: "", website: "", message: "" }); // Clear form
  };

  // Replace with your actual Google Maps Embed API Key
  const GOOGLE_MAPS_API_KEY = "YOUR_GOOGLE_MAPS_API_KEY";
  // Coordinates for Barclays Center, Brooklyn, NY (from image_60c167.jpg)
  const MAP_LATITUDE = 40.6826;
  const MAP_LONGITUDE = -73.9754;
  const MAP_ZOOM = 15; // Adjust zoom level as needed
    ...
    <div className="bg-white min-h-screen">
          ...
          <div className="flex flex-col lg:flex-row gap-12 lg:gap-16 items-start">
            <div className="w-full lg:w-1/2">
              <h2 className="text-3xl md:text-4xl font-bold text-primary leading-tight mb-6">
                We Are Always Available For <br className="hidden md:block" />{" "}
                You & Your Pets
              </h2>
              <p className="text-gray-600 mb-8 max-w-lg">
                Maecenas quis viverra metus, et efficitur ligula. Nam coueaugue
                congue sed luctus lectus conin onondimentum.
              </p>
            ...
            <div className="w-full lg:w-1/2 bg-newGray rounded-xl shadow-lg p-6 md:p-10">
              ...
              <form onSubmit={handleSubmit} className="space-y-4">
                ...
                <button className="btn-bubble btn-bubble-primary" type="submit">
                  <span>
                    Send Us Message <ArrowRight size={20} className="ml-2" />
                  </span>
                </button>
              </form>
            </div>
      ...
      <section className="w-full h-[400px] md:h-[500px] lg:h-[600px] bg-gray-200 relative">
        <iframe
          ...
          src={`https://www.google.com/maps/embed/v1/place?key=${GOOGLE_MAPS_API_KEY}&q=Barclays+Center,Brooklyn,NY&center=${MAP_LATITUDE},${MAP_LONGITUDE}&zoom=${MAP_ZOOM}`}
          title="Google Map of Barclays Center"
        ></iframe>
      </section>
    </div>
  );
